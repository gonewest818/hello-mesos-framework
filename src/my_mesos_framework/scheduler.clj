(ns my-mesos-framework.scheduler
...
(defn jar-task-info
  [uuid {:keys [slave-id]}]
  [{:name "my-mesos-framework"
    :task-id uuid
    :slave-id slave-id
    :resources {:cpus min-cpu
                :mem min-mem}
    :executor {:executor-id "my-mesos-framework-executor"
               :command {:shell true
                         :value "java -jar /vagrant/target/uberjar/my-mesos-framework-0.1.0-SNAPSHOT-standalone.jar -m my-mesos-framework.system executor" }}}])
